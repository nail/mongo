...
namespace mongo {
    ...
    */
    ...
    bool CmdAuthenticate::run(const string& dbname , BSONObj& cmdObj, int, string& errmsg, BSONObjBuilder& result, bool fromRepl) {
        ...
        // We hard code MONGODB-CR here because that's the only auth mechanism we
        // there is until we support ssl.
        audit::logAuthentication(ClientBasic::getCurrent(), "MONGODB-CR", user, status.code());
        if (!status.isOK()) {
